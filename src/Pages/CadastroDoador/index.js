...
function Doador(){
    ...
    async function handleSubmit(e){
       ...
       try {
        if(!nome || !sobrenome || !email || !rg ||!cidade || !telefone || !bairro || !numero_casa || !cep || !disponibilidade || !estado || !rua || !cpf || !senha){
            alert('preencha todos os campos')
           }

           const response = await axios.post('/doador/save',{
            nome:nome,
            sobrenome:sobrenome,
            email:email,
            rg:rg,
            ...
            <div className="card">
                ...
                <form className="form-card" onSubmit={handleSubmit}>
                    <div className="row justify-content-between text-left">
                        ...
                        <div className="form-group col-sm-6 flex-column d-flex"> <label className="form-control-label px-3">Sobrenome<span class="text-danger"> *</span></label> <input onChange={(e)=>setSobrenome(e.target.value)} type="text" id="lname" name="lname" placeholder="" onblur="validate(2)"/> </div>
                    </div>
                    <div className="row justify-content-between text-left">
                        <div className="form-group col-sm-6 flex-column d-flex"> <label className="form-control-label px-3">Email<span className="text-danger"> *</span></label> <input onChange={(e)=>setEmail(e.target.value)} type="text" id="email" name="email" placeholder="" onblur="validate(3)"/> </div>
